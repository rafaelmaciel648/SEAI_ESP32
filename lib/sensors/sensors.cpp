...
VL6180X sensor1;                    // I2C Proximity Sensor
VL6180X sensor2;
...
void sensors_setup(void){
    ...
    // I2C Setup for VL6180X - Density Sensor
    // 32 - POWER_DENS
    // 16 - INPUT SENSOR 1
    // 17 - INPUT SENSOR 2

    pinMode(DENS1_PIN, OUTPUT);
    pinMode(DENS2_PIN, OUTPUT);

    pinMode(POWER_DENS_PIN, OUTPUT);
    digitalWrite(POWER_DENS_PIN, HIGH);
    delay(1000);

    Wire.begin();

    digitalWrite(DENS1_PIN, LOW);
    digitalWrite(DENS2_PIN, LOW);
    delay(5000);

    // DENSITY 1
    digitalWrite(DENS1_PIN, HIGH);
    delay(10);
...
uint32_t turb_read(){
    uint8_t i;
    uint32_t avg=0;
    uint16_t max=0, min=65000, aux;
    for(i=0, avg=0; i<22; i++){
        aux = analogRead(TURB_PIN);
        if(aux > max){
            max = aux;
        }
        if(aux < min){
...
uint16_t dens_read(uint8_t sensor){

  uint16_t data = 420;
  if (sensor == 1){
    data = sensor1.readRangeSingleMillimeters();
    ...
    data = sensor2.readRangeSingleMillimeters();
